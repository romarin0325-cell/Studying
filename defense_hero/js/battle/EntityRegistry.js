...
export class EntityRegistry {
  ...
  add(type, entity) {
    ...
    const id = entity?.id ?? this.nextId(type.slice(0, -1));
  ...
  get(id) {
    for (const collection of [this.allies, this.enemies, this.projectiles, this.areaEffects, this.particles, this.damagePopups, this.barricades]) {
  ...
  snapshot() {
    ...
    return {
      ...
      barricades: convert(this.barricades),
    };
  }
}
