...
export class FormationScreen {
  ...
  render() {
    ...
    this.root.innerHTML = `<section class="screen formation-screen" data-screen="formation">
      ...
      <section class="roster-section"><div class="section-title"><h2>믿음직한 동료들</h2><span>4명 선택 · 눌러서 교대</span></div><div class="hero-grid hero-grid--normal">${NORMAL_HEROES.map((h) => this.card(h, this.heroIds.includes(h.id))).join("")}</div></section></div>
  ...
  openInfo(id) {
    ...
    detail.textContent = `${ATTACK_LABELS[hero.attack.attackType]} · ${hero.attack.interval}초 간격 · 사거리 ${range}`;
