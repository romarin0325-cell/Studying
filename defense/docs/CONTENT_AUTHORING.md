...
이 문서는 `defense/js/content/`의 데이터를 안전하게 수정하는 방법을 설명한다. 현재 구현과 검증 코드를 기준으로 작성했으며, 콘텐츠 파일만 바꾸면 되는 경우와 런타임·검증 코드까지 함께 바꿔야 하는 경우를 구분한다.
...
`dist-local/HeroCoreDefense.html`은 빌드 산출물이다. 직접 수정하지 말고 원본 콘텐츠·에셋을 수정한 뒤 다시 빌드한다.
...
- `scripts/verify_defense.js`
- `scripts/build_defense_local.mjs`
...
node --test defense/tests/unit/traits.test.mjs defense/tests/unit/aura-status.test.mjs defense/tests/unit/assets.test.mjs defense/tests/unit/effects-renderer.test.mjs defense/tests/unit/shotgun-stages.test.mjs defense/tests/integration/content-architecture.test.mjs defense/tests/integration/trait-interactions.test.mjs defense/tests/integration/trait-matrix-runtime.test.mjs
...
`npm run test:defense:local`은 66개 출시 에셋을 포함한 단일 HTML을 다시 만들고 `file://` 오프라인 실행을 확인한다. PR을 열거나 작업을 완료로 표시하기 전에는 저장소 규칙에 따라 최종 `npm run verify`가 반드시 통과해야 한다.
