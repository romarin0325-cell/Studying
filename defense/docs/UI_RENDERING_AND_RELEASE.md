...
- 전투 영역은 상단 12×12(y 0~11). `#drawBoard`는 y ≥ 12 셀을 어둡게 칠하고, 논리 좌표 `(0,12)→(12,12)` 구분선을 그려 랜드스케이프에서도 밴드 경계를 따른다. 실제 HUD는 캔버스 밖 HTML이며 이 4행은 시각적 거터다.
- 배치 페이즈(PREPARATION/INTERMISSION)에는 `placementCells` 화이트리스트를 초록 테두리로 하이라이트하고, 나머지 비경로 셀은 어둡게 딤 처리한다.
- 영웅 sprite(`#drawHeroes`)와 보스 sprite(`#drawEnemies`)는 `clampSpriteToBoard(dest, boardRect)`로 보드 사각형 네 변 밖 삐져나감을 클램프한다.
- StageSelectScreen은 스테이지 카드마다 10웨이브 칩 스트립(`wave-strip`)을 그리고, 칩 탭 시 적 이름·방어타입·수량 팝오버(`wave-popover`)를 표시한다. 바깥 클릭·Esc·리사이즈·스크롤을 처리한다. 정적 `STAGES` 기반이라 저장소와 무관하다.
...
모바일에서는 PC의 LAN IP와 port를 사용한다. 방화벽과 같은 네트워크인지 별도로 확인한다. server는 `defense/` 아래의 GET/HEAD만 제공하고 cache를 끄며 path traversal을 차단한다.
...
[`build_defense_local.mjs`](../../scripts/build_defense_local.mjs)는 다음을 수행한다.
...
5. [`dist-local/HeroCoreDefense.html`](../dist-local/HeroCoreDefense.html) 생성
