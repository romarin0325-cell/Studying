...
이 문서는 `defense/`를 처음 수정하는 사람이 안전하게 구조를 파악하고, 변경 종류에 맞는 검증까지 끝낼 수 있도록 만든 시작점이다. 현재 앱은 V2 코드베이스를 활성 Defense로 승격한 것이며, 기존 구현 폴더는 제거되었다. V1 저장 데이터는 현재 저장 계약과 분리되어 있다.
...
브라우저에서 `http://127.0.0.1:4174/`를 연다. 원본은 ES module이므로 `defense/index.html`을 `file://`로 직접 열지 않는다. 파일 하나로 실행해야 할 때는 다음 명령으로 오프라인 배포본을 만든다.

```powershell
npm run build:defense-local
```

생성물은 [`dist-local/HeroCoreDefense.html`](../dist-local/HeroCoreDefense.html)이다. 이 파일은 생성물이므로 직접 수정하지 않는다.
...
`test:defense`에는 모든 60개 편성×2개 스테이지의 실제 60Hz 시뮬레이션이 포함되어 있어 다른 명령보다 오래 걸릴 수 있다.
...
| 변경 | 최소 검증 | 추가 확인 |
| --- | --- | --- |
| 콘텐츠 값, trait, status | `npm run lint:defense`, 관련 unit/integration test | 밸런스 영향이면 전체 Defense test |
| BattleSession 또는 system | 전체 `npm run test:defense` | 결정론·체크포인트 연속성 |
| CSS, screen, viewport, renderer | unit/integration + `npm run test:defense:browser` | 6개 필수 viewport와 회전 |
