...
const Logic = {
    ...
    calculateInitialStats: function (playerProto, deck, allCards, idx) {
        ...
        if (t.type === 'dessert_kingdom_synergy_boost') {
            const dessertCards = ['candy_boy', 'marshmallow', 'cotton_candy_sheep', 'cream_maid', 'pudding_princess', 'harmonius', 'joker'];
            let count = 0;
            deck.forEach(id => { if (id && dessertCards.includes(id)) count++; });
            count = Math.max(0, count - 1); // Exclude self
            if (count > 0) {
