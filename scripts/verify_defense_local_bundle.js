...
async function main() {
  assert.ok(fs.existsSync(BUNDLE_PATH), "Defense single HTML is missing; run npm run build:defense-local first.");
  const outputEntries = fs.readdirSync(path.dirname(BUNDLE_PATH), { withFileTypes: true });
  assert.deepEqual(outputEntries.map((entry) => entry.name), ["HeroCoreDefense.html"]);
