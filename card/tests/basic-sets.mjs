...
try {
  ...
  const fifteenth = page.locator('.card-pool-extra-grid .card-pool-card-row:not(.is-selected)').first();
  const fifteenthId = await fifteenth.getAttribute('data-card-id');
  const fifteenthCard = page.locator(`.card-pool-extra-grid [data-card-id="${fifteenthId}"]`);
  await fifteenthCard.scrollIntoViewIfNeeded();
  const beforeLimit = await fifteenthCard.boundingBox();
  await fifteenthCard.locator('.card-pool-row-action').click();
  ...
  assert.deepEqual(await fifteenthCard.boundingBox(), beforeLimit);
  await fifteenthCard.locator('.card-pool-row-action').click();
