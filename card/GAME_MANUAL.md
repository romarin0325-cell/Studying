...
2. 플레이어 행동: `BattleRuntime.executeSkill` → `Logic.calculateDamage`. MP 검사, 치명/속성/관통/가드 감소, 부가효과. 이 경로에는 회피·배리어·매직가드 판정이 없습니다.
3. 적 턴(`TurnManager.startEnemyTurn`): 보스 AI/방어 프로토콜 후, 물리·마법 공격만 회피 → 배리어(물리 무효) → 매직가드(마법 무효) → 가드 순으로 따로 처리합니다. 회피/배리어/매직가드가 막으면 그 공격은 종료됩니다. 대미지는 `calculateDamage`가 아니라 `val * mult * (100 / (100 + def))`입니다.
...
| `artifact_chaos` | 아티팩트카오스 | 0 | 24 | 카오스 + 스테이지마다 아티팩트 4개. 챌린지 전용. 패배 시 런 삭제 |
| `draft` | 드래프트 | 5 | 24 | 뽑기 대신 드래프트. 패배 시 런 삭제 |
| `factory` | 팩토리 | 20 | 24 | 40장 번들 드래프트. 패배해도 런 저장은 유지 |
...
| `dream_corridor` | 꿈의회랑 | 20 | 무한 | 히든. 실전마법연습 3회 후 출현. TOEIC 세트에서 2문제 이상 오답 시 오답 1회, 3회째 종료 |
...
꿈의회랑은 전투 후 퀴즈가 자동으로 이어지고, 오답은 즉시 저장됩니다. TOEIC Part 5·6·7은 한 세트에서 2문제 이상 틀릴 때 오답 1회로 기록하며, 세 번째 오답 또는 전투 패배 시 런이 종료됩니다.
...
해금 판정은 **엔드리스이면서 처치한 스테이지가 30을 넘을 때만** 굴립니다(`gameType === 'endless'` 그리고 `stageNumber > 30`). 그 이전이거나 챌린지면 확률은 0입니다. 조건을 통과한 뒤 그 외 신기는 1%, `divine_astea`는 0.1%입니다.
...
- [ ] 적 공격만 회피 → 배리어/매직가드/가드. 플레이어 공격은 `calculateDamage`
- [ ] 필드버프 상한 3/5와 `valentine`/`arena`/`destiny_oath` 포함
- [ ] MP 자동회복 없음
- [ ] 카오스 룰렛/초월/퀴즈 보상 분기
- [ ] 아티팩트 26종 + 신기 대체 규칙(엔드리스 30스테이지 초과에서만 해금)
