...
const card = path.resolve(root, '../card');
const read = name => fs.readFile(path.join(root, name), 'utf8');
const source = await fs.readFile(path.join(card, 'index.html'), 'utf8');
...
const code = await Promise.all(dependencies.map(name => fs.readFile(path.join(card, name), 'utf8')));
const css = source.match(/<style>([\s\S]*?)<\/style>/)[1];
const musicCSS = await fs.readFile(path.join(card, 'music_player.css'), 'utf8');
