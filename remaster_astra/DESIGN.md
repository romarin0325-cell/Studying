...
- The repository’s `shooter/` — inspected its source and single-file builder as the delivery reference. ASTRA keeps the Card rules and data while giving them a new presentation layer.

## Fixed concept: a celestial archive

The player explores a calm observatory above the clouds. Their cards are the main objects, with a small amount of astronomical detail around them. The requested cuteness is expressed with soft corners, gentle blue, a small four-point star, friendly empty states, and quiet transitions. Avoid emoji-driven navigation, competing neon borders, constant rarity glow, ornamental particle effects and small labels on every control.
...
The serif ASTRA wordmark gives the game an identity; Korean UI uses local system sans-serif fonts for offline availability. Navigation labels are Korean. Small English labels are secondary visual texture and are never the only way to understand an action.
