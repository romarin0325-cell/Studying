...
- `MAX_MP = 100`
- 자동 MP 회복 상수는 존재하지만 현재 실전 루프에서는 사용되지 않는다
- 대미지 방어식의 방어 상수는 `100`
- `burn`/`divine`만 스택형 상태다

## 1. 전투 시작 시 생성되는 전투 객체

### 아군

전투 진입 시 각 덱 슬롯은 아래 순서로 전투 인스턴스로 복사된다.

1. 원본 카드 스탯 복사
2. `calculateInitialStats()`로 시너지/배치 보정
3. 혼돈의 축복 배수 적용
4. 위치 특성(`pos_`) 추가 보정
5. 전투용 `skills`, `buffs`, `proto` 연결
...
- `gale` 필드버프는 이미 존재할 때 `expiresAtTurn`을 갱신한다. PR #317 이후 `실피드`와 `질풍노도` 아티팩트는 이 규칙을 사용한다.
