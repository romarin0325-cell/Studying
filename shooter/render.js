...
export async function loadArt() {
  const sources = globalThis.ASTRAL_ASSETS || { heroes: 'assets/heroes.png', bosses: 'assets/bosses.png', enemies: 'assets/enemies.png', worlds: 'assets/worlds.jpg' };
  const images = {};
  ...
  const result = { heroes: [], bosses: [], enemies: [], worlds: [], urls: { heroes: [], bosses: [], worlds: [] } };
  for (const kind of ['heroes', 'bosses', 'enemies']) {
    for (let i = 0; i < 4; i++) {
      const sheet = images[kind], w = sheet.width / 2, h = sheet.height / 2;
      const sw = Math.floor(w), sh = Math.floor(h), raw = canvas(sw, sh), rc = raw.getContext('2d', { willReadFrequently: true });
      rc.drawImage(sheet, (i % 2) * w, Math.floor(i / 2) * h, w, h, 0, 0, sw, sh);
      ...
      const cut = canvas(384, 384), c = cut.getContext('2d'); c.drawImage(raw, 0, 0, 384, 384); result[kind].push(cut);
...
export class Renderer {
  ...
  render(g, dt = 1 / 60) {
    ...
    for (const e of g.enemies) {
      ...
      const size = e.boss ? (g.stageIndex === 3 ? 184 : 164) : e.elite ? 84 : 55;
      this.sprite(e.boss ? this.art.bosses[e.image] : this.art.enemies[e.image], e.x, e.y + bob, size, e.boss ? Math.sin(e.age) * .025 : Math.sin(e.age * 2) * .08, 1, e.flash > 0 ? .63 : 1);
  ...
  drawBullet(b) {
    ...
    this.c.drawImage(sprite, b.x - 14, b.y - 14);
  }
  drawPickup(d, t) {
