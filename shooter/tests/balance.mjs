...
export function pilot(g) {
  if (g.phase === 'quiz') { g.completeQuiz(g.room===2?'score':g.player.lives < g.maxLife ? 'life' : 'bomb'); return; }
  if (!['wave', 'boss'].includes(g.phase)) return;
  const p = g.player, target = g.boss?.hp > 0 ? g.boss : g.nearest(p.x, p.y);
  let best = null, bestCost = Infinity;
  const goalY = ['melee','orbit'].includes(g.weapon) && target ? Math.min(g.height * .75, target.y + 115) : g.height * .68;
