...
test('boss combat resets combo, ignores its multiplier and adds exact time and rank bonuses at victory',()=>{
  ...
  assert.equal(g.score,15000);assert.equal(g.combo,0);assert.equal(g.bullets.length,0);
  g.phase='quiz';g.room=2;g.completeQuiz();assert.equal(g.timeBonus,15000);assert.equal(g.rankBonus,4000);assert.equal(g.score,34000);
  for(const [seconds,deaths,timeBonus,rankBonus] of [[30,1,10000,2000],[40,4,5000,0],[40.01,0,0,4000],[50.01,0,0,4000]]){
...
test('hidden heroes never unlock into normal rotation; random can draw every hidden hero',()=>{
  ...
  for(let i=0;i<9;i++)assert.equal(randomHero(p,()=>(i+.5)/9,date),i);
});
