...
- Review regressions: Jasmine cannot recover a life-paid mask bomb; an active bomb cannot overlap; 100 mask attempts produce at most three successful uses per run, without resetting between stages. Permanent plus conditional attack bonuses produce 180 rather than 195 for nail + crystal + chocolate. Legacy `relaxed` settings migrate to `easy`.
