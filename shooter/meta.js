...
export const DIFFICULTIES = [
  { id: 'easy', name: '쉬움', hp: .76, speed: .78, interval: 1.2, lives: 4, maxLife:4, rare: .10 },
...
export function heroAvailable(profile, hero, date = new Date()) { return dailyHeroes(date).includes(hero) || profile.unlocks[hero] === dayKey(date); }
export function unlockHero(profile, hero, date = new Date()) { if (Number.isInteger(hero) && hero >= 0 && hero < 6) profile.unlocks[hero] = dayKey(date); }
