...
assert.ok(html.includes('globalThis.ASTRAL_EMBEDDED_ASSETS'), 'Offline HTML must embed asset map');
assert.ok(html.includes('data:image/webp;base64,'), 'The single-file HTML must embed preprocessed WebP data URLs');
assert.ok(!html.includes('data:image/png'), 'Must not embed raw PNG images');
assert.ok(Buffer.byteLength(html) < 20 * 1024 * 1024, 'Single HTML must be under 20 MiB');
