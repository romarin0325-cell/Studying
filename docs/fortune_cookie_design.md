...
| **MP3 파일 형태** | 문제별 개별 파일 |
| **기초 강의 필요성** | 미결정 (아래 Open Questions 참고) |
| **AI 해설 기능** | 미결정 (아래 Open Questions 참고) |
...
기존 게임에서 문법은 `grammar_data.js`로 강의 데이터를 가지고 있고,
실전마법연습(TOEIC)도 Part 5/6/7 리딩 문법 기반입니다.

**리스닝도 기초 전략 강의가 필요할지?**

#### 선택지

**A) 기초 강의 불필요 — 바로 문제 풀기**
- 포춘쿠키는 "운세"가 메인이므로, 리스닝은 가볍게 풀어보는 느낌
- Part별 간단한 팁만 모달에 표시 (예: "Part 3은 두 사람의 대화입니다. 핵심 키워드에 집중하세요.")
- 구현 비용 ↓

**B) 기초 강의 콘텐츠 추가**
- `listening_strategy_data.js` 같은 형태로 Part별 전략/요령 데이터 추가
- 처음 사용 시 "리스닝 가이드 보기" 옵션 제공
- 구현 비용 ↑, 콘텐츠 제작 필요

**C) 교재 학습 후 결정**
- 교재를 구매해서 리스닝 강의를 먼저 들어본 뒤, 어떤 전략이 도움이 되는지 파악 후 결정
- 1차 구현에서는 문제만 제공하고, 전략 콘텐츠는 추후 추가

### Q2. AI 해설 기능 범위

기존 시스템의 AI 해설 기능 참고:
- 실전마법연습: `TOEIC_EXPLANATIONS`로 **정적 해설 텍스트** 제공
- 루미 질문하기: Part 6/7에서 **AI(Gemini) 기반 질문** 기능
- 두 시스템 모두 이미 구현되어 있어 패턴 재활용 가능

**리스닝 문제에도 해설/AI 질문 기능이 필요할지?**

#### 선택지

**A) 해설 없음 — 정답만 표시**
- 포춘쿠키의 주목적은 운세이므로, 문제는 "운세를 열기 위한 열쇠" 역할
- 운세 화면에 정답/오답만 표시
- 가장 단순

**B) 정적 해설 텍스트**
- `listening_data.js`에 각 문제별 해설 필드 추가
- 운세 화면 아래에 "해설 보기" 토글
- 교재 해설을 직접 입력해야 하므로 콘텐츠 작업 필요

**C) AI 해설 (루미 질문하기 연동)**
- 기존 `LumiQuestionRuntime` 패턴을 재활용
- 운세 화면에서 "루미에게 질문하기" 버튼
- 리스닝 문제/보기/정답/사용자 답을 컨텍스트로 전달
- Gemini API 키 필요 (이미 기존에 연동됨)
- Part 6/7 루미 질문하기와 동일한 UX
